    ...
    startTimer = ()=> {
        ...
        if(endTime.getTime() - now){
            if(this.props.isVisible){
                var wraning = !(parseInt(timer.hour)) && parseInt(timer.minute) < 30;
                if(this.props.num < 3){
                    this.setState({
                       timer: `${timer.hour}:${timer.minute}:${timer.second}''`,
                        warning: warning
                    });
                } else if(now - this.lastUpdateTime > 1000 * 10){
        ...
        return (
            <View style={[styles.container, {borderBottomWidth: this.props.isLast?0:1}]}>
                <TouchableHighlight
                    ...
                    opPress={this.handleCompleted}>
                    <Image style={styles.btnIcon}
                        source={this.getCompleted()?require('./img/checked_filled.png'):require('./img/checked.png')}/>
                </TouchableHighlight>
                <TouchableHighlight
                    activeOpacity={Theme.active.opacity}
                    underlayColor="transparent"
                ...
                <Text style={[styles.timer, {color: this.state.warning &&!this.getCompleted()? Theme.color.red: '#000'}]}>
                    {this.getCompleted() ? 'completed': this.state.timer}
                </Text>
            </View>
        );
    }
